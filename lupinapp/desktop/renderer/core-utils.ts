...
const FONT_FAMILY_PRESET_CSS: Record<RendererFontFamilyPreset, string> = {
  site: "",
...
function parseLetterSpacingValue(value: unknown, fontSize = 18): number {
...
function formatLetterSpacingEm(value: number): string {
  return `${Number(clamp(value, -0.08, 0.2).toFixed(3))}em`;
}

function normalizeRendererSettings(raw: Partial<RendererSettings> | null | undefined): RendererSettings {
...
function resolveFontFamilyPreset(
  preset: RendererFontFamilyPreset,
  fallbackFontFamily = ""
): string {
  if (preset === "site") {
...
function buildTypographyPresetFromSettings(
  settings: RendererSettings,
  fallback: RendererTypographyPreset | null = null
): RendererTypographyPreset {
  return {
    ...
    letterSpacing: formatLetterSpacingEm(
      Number.isFinite(Number(settings.letterSpacing))

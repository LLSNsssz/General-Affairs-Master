...
const READER_PRELOAD_SITE_RULES = [
    {
        id: "novelpia",
        hosts: ["novelpia.com"],
...
function normalizeReaderPreloadUrl(url) {
    return String(url || "").toLowerCase();
}
function getReaderPreloadSiteRule(url) {
...
function isMunpiaPreloadReadingUrl(url) {
    const value = normalizeReaderPreloadUrl(url);
...
function isReaderPreloadReadingPage(siteId, url) {
    const value = normalizeReaderPreloadUrl(url);
...
function hideMunpiaChromeEarly() {
    ...
    const looksLikeThinDivider = (node, rect, text, style) => {
        const descriptor = [
            node.tagName,
            node.className,
            node.id,
...
function hideNovelpiaInlineOverlays() {
    ...
    const readerRoot = (document.getElementById("novel_drawing_page")
...
function runReaderPreloadMaintenance() {
...
function scheduleReaderPreloadMaintenance(delay = 80) {

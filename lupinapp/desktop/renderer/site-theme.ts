type ReaderSiteId = "novelpia" | "munpia" | "kakaopage" | "joara";

type ReaderSiteUrlStrategy = {
  id: ReaderSiteId;
  hosts: string[];
  readingTokens: string[];
  useMinimalAmbientTheme?: boolean;
};

const READER_SITE_URL_STRATEGIES: ReaderSiteUrlStrategy[] = [
  {
    id: "novelpia",
    hosts: ["novelpia.com"],
...
function matchesAnyUrlToken(value: string, tokens: string[]): boolean {
  return tokens.some((token) => value.includes(token));
}

function getReaderSiteStrategy(url: string): ReaderSiteUrlStrategy | null {
...
function getReaderSiteId(url: string): ReaderSiteId | null {
...
function isMunpiaReadingUrl(url: string): boolean {
  const value = normalizeReaderUrl(url);
...
function buildMemoPaperBackgroundCss(): string {
  return `
    :root {
      --sticky-paper: #f6efb6;
...
function buildReaderComfortCss(url: string, typography?: RendererTypographyPreset | null): string {
  if (!isReadingUrl(url)) {
    return "";
  }
...
function buildSiteDomAdapterScript(url: string, typography?: RendererTypographyPreset | null): string {
  const siteId = getReaderSiteId(url);

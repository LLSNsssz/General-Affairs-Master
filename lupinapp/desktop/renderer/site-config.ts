...
function getPresetHomeUrl(site?: SitePreset | null): string {
  return site?.mobileUrl || site?.url || "";
}

function getPresetLoginUrl(site?: SitePreset | null): string {
  return site?.mobileLoginUrl || site?.loginUrl || getPresetHomeUrl(site);
}

function getPresetMeta(site: SitePreset): string {

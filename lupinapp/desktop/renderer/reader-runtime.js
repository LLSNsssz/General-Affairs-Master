...
function shouldAdoptNovelpiaTypographyDefaults() {
    const settings = normalizeRendererSettings(state.settings);
    return (!settings.initializedFromNovelpia
...
function getGlobalReaderTypography() {
...
function shouldBlockOnSiteAdapter(url) {
    return isMunpiaUrl(url) && isReadingUrl(url);
}
function shouldApplySiteAdapter(url) {
    const value = String(url || "").toLowerCase();
    return (shouldBlockOnSiteAdapter(url)
        || (value.includes("novelpia.com") && !isAuthPage(url)));
}
function shouldScheduleAdapterRefreshes(url) {
...
function buildSimplifyScript(typography) {

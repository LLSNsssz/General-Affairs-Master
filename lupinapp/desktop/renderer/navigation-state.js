...
function mergePresetBookmarks(savedBookmarks) {
    const remainingBookmarks = Array.isArray(savedBookmarks)
        ? [...savedBookmarks]
        : [];
    ...
    return mergedPresets.concat(filteredBookmarks
        .filter((bookmark) => Boolean(bookmark && bookmark.url))
        .map((bookmark) => ({
...
function getInitialUrl() {
    ...
    if (normalizedStoredUrl && /^https?:/i.test(normalizedStoredUrl) && !normalizedStoredUrl.startsWith("chrome-error://")) {
        ...
        if (normalizedStoredUrl.includes("/proc/login_kakao") ||

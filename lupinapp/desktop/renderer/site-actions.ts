...
function goBackInReaderHistory(): void {
...
function getCurrentReaderUrl(): string {
  return (readerController.getURL() || state.currentUrl || "").toLowerCase();
}

type ReaderOverlayDebugEntry = {
  selector: string;
  role: string;
  position: string;
  zIndex: string;
...
async function toggleReaderDevTools(): Promise<void> {
  ...
  try {
    ...
    const report = await readerController.executeJavaScript<ReaderOverlayDebugEntry[]>(
      buildReaderOverlayDebugScript(),
      true
    );
...
async function dismissMunpiaOverlayIfPresent(): Promise<boolean> {
  if (!readerController.isAttached()) {
    return false;
  }

  try {
    return Boolean(await readerController.executeJavaScript<boolean>(buildMunpiaDismissOverlayScript(), true));
  } catch (error) {
    console.error(error);
    return false;
  }
...
function isNovelpiaReaderPage(url: string): boolean {
  return url.includes("novelpia.com") && isReadingUrl(url);
}

function isMunpiaReaderPage(url: string): boolean {
  return url.includes("munpia.com") && isReadingUrl(url);
}

function getReaderActionStatusLabel(action: ReaderActionShortcut): string {
  switch (action) {
...
function getUnsupportedReaderActionMessage(action: ReaderActionShortcut, url: string): string {
...
async function runReaderActionShortcut(action: ReaderActionShortcut): Promise<void> {
  setHelpPopupOpen(false);
  ...
  try {
    const didRun = await readerController.executeJavaScript<boolean>(script, true);
...
function buildNovelpiaReaderActionScript(action: ReaderActionShortcut): string {
  return `
    (() => {
...
function buildMunpiaReaderActionScript(action: ReaderActionShortcut): string {
  if (action !== "episode-list") {
    return "";
  }
...
function buildMunpiaDismissOverlayScript(): string {
  return `
    (() => {
...
function buildReaderOverlayDebugScript(): string {
  return `
    (() => {
      function isVisible(node) {
        if (!(node instanceof HTMLElement)) {

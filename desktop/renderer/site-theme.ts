...
function isSupportedNovelSite(url: string): boolean {
  const value = String(url || "").toLowerCase();
  return [
    "novelpia.com",
    "page.kakao.com",
...
function isMunpiaUrl(url: string): boolean {
  return String(url || "").toLowerCase().includes("munpia.com");
}

const READING_FONT_OFFSET_PX = 1;

function buildMunpiaSurfaceCss(typography?: RendererTypographyPreset | null, applyReaderTypography = false): string {
...
function shouldUseMinimalAmbientTheme(url: string): boolean {
  const value = String(url || "").toLowerCase();
  return [
    "joara.com"
  ].some((token) => value.includes(token));
...
function shouldUseCatalogAmbientTheme(url: string): boolean {
  return isSupportedNovelSite(url) && !isAuthPage(url) && !isReadingUrl(url);
}

function isReadingUrl(url: string): boolean {
  const value = String(url || "").toLowerCase();
  if (!value) {
    return false;
  }
...
function buildReaderChromeSuppressionCss(url: string): string {
  if (!isReadingUrl(url)) {
    return "";
  }
...
function buildCatalogShellCss(url: string): string {
...
function buildReaderComfortCss(url: string): string {
  if (!isReadingUrl(url)) {
    return "";
  }
  ...
  return `
    ...
    article p,
    main p {
      font-size: calc(1em + ${READING_FONT_OFFSET_PX}px) !important;
...
function buildPopupCloseVisibilityCss(): string {
  return `
...
function buildModalSafetyCss(url: string): string {
  const value = String(url || "").toLowerCase();
  ...
  return `
    @media (max-width: 460px), (max-height: 420px) {
      html,
      body {
      ...
      [id*='layer'] {
        ...
        background: #f6efb6 !important;
        box-shadow: none !important;
        border: 1px solid rgba(120, 109, 36, 0.3) !important;
      ...
      [id*='overlay'] {
        position: fixed !important;
        inset: 0 !important;
        background: rgba(246, 239, 182, 0.82) !important;
      ...
      [role='button'][id*='close' i] {
        ...
        background: rgba(255, 248, 196, 0.94) !important;
        border: 1px solid rgba(120, 109, 36, 0.36) !important;
      }
    }
  `;
}

function buildSiteDomAdapterScript(url: string): string {
  const value = String(url || "").toLowerCase();
    ...
    return `
      (() => {
        ...
        function collectReadableBlocks(root, titleBottom) {
          const results = [];
          const seen = new Set();
          const selectors = ["p", "blockquote", "dd", "li", "td", "div"];
        ...
        function findContentRoot(titleBottom) {
          ...
          [
            ...
            document.querySelectorAll(selector).forEach((node) => {
              ...
              const previewChars = blocks.slice(0, 12).reduce((sum, block) => sum + block.text.length, 0);
        ...
        function applyMunpiaReaderFallback(titleElement) {
          ...
          const titleRect = titleElement.getBoundingClientRect();
          const firstBlockElement = findFirstReadableBlockElement(titleRect.bottom);
        ...
        function ensureReaderStyles() {
          ...
          style.textContent = [
            ...
            "body[data-sticky-munpia-reader='1'] > *:not(#sticky-munpia-reader-root):not(script):not(style) { visibility: hidden !important; }",
            ...
            "body[data-sticky-munpia-back='1'] #sticky-munpia-back-chip, body[data-sticky-munpia-back='1'] #sticky-munpia-next-chip { opacity: 1; pointer-events: auto; }"
          ].join("\\n");
        ...
        function ensureBackChip() {
          ...
          document.addEventListener("pointerdown", (event) => {
            ...
            if (target instanceof HTMLElement && target.closest("#sticky-munpia-back-chip, #sticky-munpia-next-chip")) {
              return;
            }
            showChip();
          }, true);
        ...
        function renderMunpiaReaderView() {
          ...
          const titleRect = titleElement.getBoundingClientRect();
          applyMunpiaReaderFallback(titleElement);
          ensureBackChip();
          ...
          const firstBlockElement = contentCandidate.blocks[0]?.element;
          if (firstBlockElement instanceof HTMLElement) {
            hideMunpiaInlineChrome(titleElement, firstBlockElement);
          }
  ...
  if (false && value.includes("munpia.com") && !isAuthPage(url) && !isReadingUrl(url)) {
    return `
      (() => {
